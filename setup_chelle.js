...
async function setupChelle() {
  const email = 'chellewigss@gmail.com';
  const name = 'Chelle';
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.log('Successfully sent invite email to Chelle via API!');
    }
  } catch (err) {
  ...
  if (insertError) {
    ...
    console.log('Successfully inserted Chelle into staff table!');
  }
}
